...
class BookList extends Component {
	...
	validationEmail = (e) => {
		if (e.target.value.length > 0) {
			if (e.target.value.length < 30) this.setState({ title: e.target.value, error: '' })
			else this.setState({ error: 'Title is too long' })
		} else this.setState({ title: e.target.value, error: 'Title is too short' })
	}

	validationAuthor = (e) => {
		if (e.target.value.length < 20) this.setState({ author: e.target.value, error: '' })
		else this.setState({ error: 'Title is too author' })
	}
